...
## Observer data
```{r}
#Figuring out the new observer pull - 10/2/2022
observer1 <- read_excel("./data/22-029_Cadrin_OB.xlsx", sheet = 2)

observer2 <- read_excel("./data/22-029_Cadrin_OB.xlsx", sheet = 3)


#just making sure github is wokring correctly 
...
###############################################################################################################
############################################# OLD OBSERVER DATA READIN ######################################

#Figuring out what the deal is with all the NAs in the NETTYPE 
observer1 <- read_csv('./data/result1.csv')

observer11 <- read_csv('./data/result1.csv', 
                       col_types = list(
                        NETTYPE = col_double(),
                        NSWEEPS = col_double()))

observer12 <- read_excel('./data/result1.xlsx')
...
observer21 <- read_csv('./data/result2.csv', 
                       col_types = list(
                        NETTYPE = col_double(),
                        NSWEEPS = col_double(), 
                        PERMIT1 = col_double()))

observer22 <- read_excel('./data/result2.xlsx')
...
observer2 <- observer2 %>%
  mutate(PERMIT1 = as.logical(PERMIT1), 
         GIS_LATHBEG = as.numeric(GIS_LATHBEG), 
         GIS_LONHBEG = as.numeric(GIS_LONHBEG),
         GIS_LATHEND = as.numeric(GIS_LATHEND), 
         GIS_LONHEND = as.numeric(GIS_LONHEND), 
         NSWEEPS = as.numeric(NSWEEPS), 
         SWEEPDIA = as.numeric(SWEEPDIA), 
         LINERMSIZE = as.numeric(LINERMSIZE), 
         CODMSIZE = as.numeric(CODMSIZE),
         MARKET = as.numeric(MARKET), 
         MSWGTAVG = as.numeric(MSWGTAVG), 
         SOAKDUR = as.numeric(SOAKDUR), 
         HAULDUR = as.numeric(HAULDUR), 
         HAILWT = as.numeric(HAILWT), 
         LIVE_WT = as.numeric(LIVE_WT)
         )

#bind the two sheets together
observer_old <- bind_rows(observer1, observer2)
...
observer_trawl_old <- observer_old %>%
  filter(NEGEAR %in% c("050", "057", "150")) %>%
      #050 - Trawl, Otter, Bottom, Fish
      #057 - Trawl, Otter, Bottom, Haddock Separator
      #150 - Trawl, Otter, Bottom, Large Mesh Belly Panel
...
## Categorizing Trawls based on Net Type
```{r}
#Categorizing observer tows
observer_trawl %>%
...
#categorizing study fleet trawls 
sf_trawl %>%
  count(VTR_GEAR_CODE)
  count(VESSEL_NAME)
...
observer_trawl %>%
  filter(PERMIT1 == 410342 & YEARHBEG == 2010)

sf_trawl %>%
  count(VESSEL_PERMIT_NUM)


sf_trawl_permit <- sf_trawl %>%
  select(YEAR, VESSEL_PERMIT_NUM) %>%
  group_by(YEAR) %>%
  distinct(VESSEL_PERMIT_NUM) %>%
  arrange(YEAR) %>%
  mutate(year_permit = paste(YEAR,VESSEL_PERMIT_NUM, sep = '_'))


observer_trawl_permit <- observer_trawl %>%
  group_by(YEARHBEG) %>%
  distinct(PERMIT1) %>%
  arrange(YEARHBEG) %>%
  mutate(year_permit = paste(YEARHBEG,PERMIT1, sep = '_'))

sf_obsv_permit_match <- inner_join(sf_trawl_permit, observer_trawl_permit, 
                                   by = "year_permit")

df2 <- inner_join(observer_trawl, sf_obsv_permit_match, 
                  by = c("PERMIT1", "YEARHBEG"))

df2 %>%
  group_by(PERMIT1, YEARHBEG) %>%
  count(NETTYPE)



sf_trawl_permits <- as.vector(n_distinct(sf_trawl$VESSEL_PERMIT_NUM))
```
...
#cheching to make sure github works on the new computer