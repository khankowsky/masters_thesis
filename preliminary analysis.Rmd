...
#import the length data
nefsc_length_spring <- read_csv("./data/22560_UNION_FSCS_SVLEN_fall.csv", 
                                col_types = list(
                                  CRUISE6 = col_double(),
                                  STRATUM = col_double(),
                                  TOW = col_double(),
...
nefsc_length_fall <- read_csv("./data/22561_UNION_FSCS_SVLEN_spring.csv", 
                                col_types = list(
                                  CRUISE6 = col_double(),
                                  STRATUM = col_double(),
                                  TOW = col_double(),
...
head(nefsc_length_fall)
head(nefsc_catch_spring)
skim(nefsc_catch_spring)
skim(nefsc_length_fall)
...
#applying the length-weight relationship to the survey data 
nefsc_length_spring_select <- nefsc_length_spring_select %>%
  ...
  summarise(sumCATCH = sum(totalWeight))
...
nefsc_catch_spring %>%
  filter(CRUISE6 > 200900) 

nefsc_catch_spring <- nefsc_catch_spring %>%
  select(-c("...11","...12"))


nefsc_length_spring_select %>%
  filter(CRUISE6 > 200900) %>%
  filter(SVSPP %in% c(73, 74))

#now join this with the catch data and subtract the small fish from the total weight 
nefsc_catch_spring_updated <- left_join(nefsc_catch_spring, nefsc_length_spring_select, 
                                        by = c("ID", "CRUISE6", "STRATUM", "TOW", "STATION", "SVSPP"))



#subtracting the small fish from the total weight
nefsc_catch_spring_updated %>%
  filter(CRUISE6 > 200900) %>%
  filter(SVSPP %in% c(73, 74))
...
nefsc_length_fall %>%
  group_by(CRUISE6, STRATUM, TOW, STATION, SVSPP) %>%
  summarise(sumCATCH = sum(EXPNUMLEN))
