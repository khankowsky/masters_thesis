...
models_all_cod_old <- lapply(datasets_tibble$data, fit.models_old)
...
compared_all_old <- lapply(models_all_cod_old, compare.models_old)


compare.models <- function(x, y) {
  y %>%
  mutate(aic =  purrr::map_dbl(x, AIC)) %>%
  mutate(rsq =  purrr::map_dbl(x, Dsquared)) %>% 
  arrange(desc(aic)) %>%
...
clean_list_of_models %>%
  mutate(aic =  purrr::map_dbl(models_all_cod, AIC)) %>%
  mutate(rsq =  purrr::map_dbl(models_all_cod, Dsquared)) %>% 
  arrange(desc(aic)) %>%
  mutate(deltaAIC = aic - aic[length(models_all_cod[1:64])]) %>%
  relocate(rsq, .after = deltaAIC) %>%
  mutate(rsq = sprintf("%0.2f", rsq)) %>%
  mutate(aic = sprintf("%1.0f", aic)) %>%
  mutate(deltaAIC = sprintf("%1.0f", deltaAIC)) %>%
...
compared_all_cod <- mapply(compare.models, models_all_cod, clean_list_of_models)
compared_all_cod1 <- as.tibble(compared_all_cod)


compared_all_cod <- mapply(compare.models, models_all_cod, 1:64)

compared_all_cod <- lapply(models_all_cod, compare.models)
length(models_all_cod[1:64])


#extract the optimal model from each table 
optimal_models_old <- lapply(compared_all_old, extract_optimal_model)

optimal_models_cod


#adding a column to the dataframe of the optimal model formula for each dataset 
datasets_tibble_cod1 <- datasets_tibble %>%
...
dataset_final_cod <- datasets_tibble_cod1 %>%
...
list_of_models1 <- rep(list(formulae),64)
...
###Lu's attempt
fun.glm <- function(form, data) {
  glm(form, family = binomial, data)
}
datasets_tibble_cod <- datasets_tibble_cod %>%
  mutate(all_models = purrr::map2(list_of_models, data, map2(list_of_models, ~fun.glm)))

 ## Lu's other attempt
 for(i in nrow(datasets_tibble_cod)){
 purrr::map(datasets_tibble_cod$list_of_models, ~glm(.x, family = binomial, data = datasets_tibble_cod$data))
 }
                                  
  ####                               
datasets_tibble_cod <- datasets_tibble_cod %>%
  mutate(all_models = purrr::map2(list_of_models, data, ~glm(.x, family = binomial, data = .y)))


  mutate(model_comparison_tbl = purrr::map2(list_of_models, all_models, compare.models))
  

datasets_tibble_cod$list_of_models[[1]][[1]]



models1 <- datasets_tibble_cod$all_models[[1]]


sapply(clean_list_of_models,"[[", c(1, 1:64))

clean_list_of_models[[1]]

models_all_cod[[1]]
models_all_cod_old[[1]]
