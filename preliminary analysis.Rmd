...
sf_trawl_slim <- sf_trawl_slim %>%
  drop_na(c(START_TOW_LAT, END_TOW_LAT, START_TOW_LAT, END_TOW_LAT)) %>%
  mutate(midpoint_lat = mean(c(START_TOW_LAT, END_TOW_LAT))) %>%
  mutate(midpoint_long = mean(c(START_TOW_LAT, END_TOW_LAT)))
...
sf_trawl_slim <- st_as_sf(sf_trawl_slim, coords = c("midpoint_long", "midpoint_lat"), 
                             crs = 4269)

ob_trawl_slim <- st_as_sf(ob_trawl_slim, coords = c("midpoint_long", "midpoint_lat"), 
                             crs = 4269)

vw_slim <- st_as_sf(vw_slim, coords = c("midpoint_long", "midpoint_lat"), 
                             crs = 4269)
...
st_contains(sf_trawl_slim, nefsc_strata)
st_contains(ob_trawl_slim, nefsc_strata)
st_contains(vw_slim, nefsc_strata)
...
paired_tows_1mi_24hr_ob <- nefsc_slim %>% 
  ...
  mutate(time_dif = difftime(START_TOW_DATE_GMT.x, START_TOW_DATE.y, units = "days")) %>%
