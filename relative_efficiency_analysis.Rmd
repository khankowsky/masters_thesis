...
compare.models <- function(x) {
  ...
  mutate(rsq =  purrr::map_dbl(x, rsq)) %>% 
...
#trying the dredge method
library(MuMIn)

cod_paired_tows_5mi_72hr_logre <- cod_paired_tows_5mi_72hr_logre %>%
  drop_na(BOTTEMP, AVGDEPTH) %>%
  select(-c(AIRTEMP, BAROPRESS, WINDDIR, WINDSP, WEATHER))

codm1 <- glm(relative_efficiency ~ season + mgmt_area  + VESSEL_NAME + AVGDEPTH + SURFTEMP + BOTTEMP, data = cod_paired_tows_5mi_72hr_logre) 
