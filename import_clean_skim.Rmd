...
## Calculating Day/Night Variable
```{r}
library(suncalc)

#Calculating whether the tow was conducted during the day or night for each dataset
#Running for the observer dataset 
observer_trawl <- observer_trawl %>%
  mutate(start_tow_date = as.Date(START_TOW_DATE)) %>%
  rowwise %>%
  mutate(sunrise = getSunlightTimes(date = start_tow_date, lat = START_TOW_LAT, lon = START_TOW_LON, tz = "EST")$sunrise, 
         sunset = getSunlightTimes(date = start_tow_date, lat = START_TOW_LAT, lon = START_TOW_LON, tz = "EST")$sunset)

observer_trawl <- observer_trawl %>%
  mutate(dayNight_ob = ifelse(START_TOW_DATE > sunrise & START_TOW_DATE < sunset, 'day', 'night'))

gc()
  
  
#Running it for the study fleet data
sf_trawl <- sf_trawl %>% 
  force_tz(START_TOW_DATE_GMT, tz = "GMT") %>%
  mutate(START_TOW_DATE_EST = with_tz(START_TOW_DATE_GMT, tz = "EST")) %>%
  mutate(start_tow_date = as.Date(START_TOW_DATE_EST)) %>%
  rowwise %>%
  mutate(sunrise = getSunlightTimes(date = start_tow_date, lat = START_TOW_LAT, lon = START_TOW_LON, tz = "EST")$sunrise, 
         sunset = getSunlightTimes(date = start_tow_date, lat = START_TOW_LAT, lon = START_TOW_LON, tz = "EST")$sunset)

sf_trawl <- sf_trawl %>% 
  mutate(dayNight_sf = ifelse(START_TOW_DATE_EST > sunrise & START_TOW_DATE_EST < sunset, 'day', 'night'))
  
gc()  


#Running it for the survey data
nefsc <- nefsc %>%
  mutate(start_tow_date = as.Date(START_TOW_DATE_EST)) %>%
  rowwise %>%
  mutate(sunrise = getSunlightTimes(date = start_tow_date, lat = START_TOW_LAT, lon = START_TOW_LON, tz = "EST")$sunrise, 
...
nefsc <- nefsc %>%
  mutate(dayNight_nefsc = ifelse(START_TOW_DATE_EST > sunrise & START_TOW_DATE_EST < sunset, 'day', 'night'))
...
sf_trawl_slim <- sf_trawl %>%
  select(c("TRIP_ID", "VESSEL_NAME", "TARGET_SPECIES", "trawl_type", 
           ...
           "sf_species_catch", "sf_species_cpua", "dayNight_sf"))
...
ob_trawl_slim <- observer_trawl %>%
  select(c("trip_haul_ID", "START_TOW_DATE", "END_TOW_DATE", "AREA", "VESSELNAME",  "NEGEAR", 
           ...
           "ob_species_catch", "ob_species_cpua", "dayNight_ob"))
         # "swept_area"))
...
nefsc_slim <- nefsc %>%
  select(c("cruise_station_ID", "STRATUM", "AREA", "SVVESSEL", "SVGEAR", 
           "START_TOW_DATE_GMT", "END_TOW_DATE_GMT", "START_TOW_DATE_EST", 
           ...
           "SWEPT_AREA", "nefsc_species_cpua", "nefsc_species_cpua_LB", "dayNight_nefsc"))
