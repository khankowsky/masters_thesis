...
sf_trawl <- sf_trawl %>% 
  ...
  mutate(Tow_Catch_LB = sum(HAIL_AMOUNT_LB)) %>%
  ungroup() %>%
  mutate(CATCH_per_SWEPT_AREA = Tow_Catch_LB/swept_area)
  

#calculates the catch per swept area for the nefsc bottom trawl survey 
nefsc <- nefsc %>%
  rename(SWEPT_AREA = AREA_SWEPT_DOORS_MEAN_KM2) %>%
  mutate(CATCH_per_SWEPT_AREA = CATCHWT_updated/SWEPT_AREA)
...
nefsc_slim <- nefsc %>%
  select(c("cruise_station_ID", "STRATUM", "AREA", "SVVESSEL", "SVGEAR", 
           ...
           "SWEPT_AREA", "CATCH_per_SWEPT_AREA"))
