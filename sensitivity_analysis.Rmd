...
fit.models_cod <- function(x, y) {
  y$formula %>% purrr::map( ~glm(.x, family = binomial, data=x))
}
...
compared_all_cod <- lapply(compare.models, models_all_cod, clean_list_of_models)
...
datasets_tibble_cod <- datasets_tibble_cod %>% 
  ...
  select(dataset, data, formula) %>%
  group_by(dataset) %>%
  nest(list_of_models = formula) %>%
  ungroup()
